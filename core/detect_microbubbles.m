...
switch lower(params.method)
    ...
    case 'fixed'
        % CORRECTED DETECTOR. Fixed absolute envelope threshold, calibrated on
        % bubble-free Bg blocks so they yield ~0 detections (calibrate_bg_floor.m).
        % Unlike 'threshold' (edge-noise * N, signal-dependent: quiet low-conc
        % frames lower the threshold and admit clutter), this is a fixed cut in
        % envelope-amplitude units, so the floor does not move with concentration.
        % See FINDINGS_2026_06_11_lowconc_floor.md.
